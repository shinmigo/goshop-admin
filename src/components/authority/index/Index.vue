<template>
    <div class="main">
        ...
        <div class="operate margin-top-df padding-right-sm text-right">
            <a-button type="primary" icon="plus" @click="add()" class="margin-right-sm">新建管理员</a-button>
            <batch-type :batchList="batchList" @refresh="refreshBatch"></batch-type>
        </div>
        <div class="pitchNumber margin-top-sm radius-xs" ><span>i</span>已选<span>{{ rowKeys.length }}</span>个管理员<span @click="emptyCheck">清空已选</span></div>
...
<script>
    ...
    export default {
            ...
            return {
                ...
                searchList:{
                    list:[/* type:1输入框，2上下架选择框，3商品分类，4启用禁用 */
                        ...
                        { label:'状态', value:'status', title:'请选择管理员状态', type: 4 },
                        { label:'管理员名称', value:'name', title:'请输入管理员名称', type: 1 },
                    ],
                    value:{
                        name:'张三',
                        phone:'',
        ...
        created () {
            ...
            this.$store.state.breadValue = '管理员列表';
        ...
        methods: {
            ...
            deleted(item){
                ...
                this.styleType.text = '删除管理员 <span class="text-red">“'+ item.name +'”</span> 后，管理员将不可使用。您确认删除吗？';
